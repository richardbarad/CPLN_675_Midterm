...
For a high level overview of this project please view the [Youtube video](https://youtu.be/EpNPwfKWg6Q).
...
```{r import_libraries, results='hide',warning=FALSE, include=FALSE}
library(sf)
library(caret)
library(pscl)
library(plotROC)
...
```{r read_files, message=FALSE, results='hide'}
...
The plots below provide another method of exploring the data. The plots present the spread of the elevation, flow accumulation, slope, and distance from water variables. As shown, the elevation tends to be lower for grid squares that flooded. Almost all grid squares that flooded are within 150 meters of the lowest elevation in the city. Additionally, the grid squares that flooded tend be located near water. Additionally, the flow accumulation tends to be higher for grid squares that flooded. 

```{r, fig.height=4,fig.width=10}

t <- grid7 %>%
  as_data_frame() %>%
  select(in_id,elevation,water_dist,fa_log) %>%
  pivot_longer(cols = -in_id,names_to='variable',values_to='value')

ggplot(t) + 
     geom_violin(aes(x = as.factor(in_id), 
                  y = value, fill = as.factor(in_id))) + 
     facet_wrap(~variable, scales = "free",labeller = as_labeller(c(elevation = "elevation", fa_log = "Flow Accumulation Natural Log", slope_max = 'Slope', water_dist= 'Distance to Water'))) +
     labs(x="Innundated", y="Value") + 
...
# Denver Data Preperation and Analysis
...
## Denver Elevation Data Preprocessing

Preprocess elevation DEM for Denver and create slope raster from DEM.
...
Extract elevation data for each grid squares, calculate mean elevation for each grid square.
...
Extract slope data for each grid squares, calculate max slope for each grid square.

```{r denver_slope_zonal}
...
```{r denver_map_predictions, fig.width=11,fig.height=4}
